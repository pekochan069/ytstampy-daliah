...
class SingingAnalysisPipeline:
    ...
    def __init__(
        ...
        )  # Placeholder, will be updated after filename generation
    ...
    def _generate_base_filename(self) -> bool:
        ...
        if source_file:
            ...
            try:
                ...
                if "youtube.com" in parsed_url.netloc and "v" in parse_qs(
                    parsed_url.query
                ):
        ...
        try:
            ...
            self._update_status(
                f"Output for this run will be in: {self.run_output_dir}"
            )
            return True
        except OSError as e:
            self._update_status(
                f"Error creating output subdirectory '{self.run_output_dir}': {e}. Using main directory."
            )
    ...
    def _load_audio(self) -> bool:
        ...
        self._update_progress(
            None, f"Loading audio source: {os.path.basename(str(source))}"
        )
        try:
            # Pass the *main* output_dir, loader might need it for initial download/conversion
            self.loader = AudioLoaderFactory.create_loader(
                source, output_dir=self.main_output_dir
            )
            ...
            )  # Keep the path to the raw/converted audio
            self.results["total_duration"] = self.loader.duration
            self._update_status(
                f"Audio loaded: {self.results['total_duration']:.1f}s @ {sr}Hz"
            )
    ...
    def _detect_segments(self) -> bool:
        ...
        try:
            ...
            non_singing_ref = (
                max(0, nsing_start),
                min(total_dur, nsing_start + ref_dur),
            )
            self._update_status(
                f"Using Singing Ref: {singing_ref[0]:.1f}-{singing_ref[1]:.1f}s"
            )
            self._update_status(
                f"Using Non-Singing Ref: {non_singing_ref[0]:.1f}-{non_singing_ref[1]:.1f}s"
            )

            segments, frame_df_with_states = self.detector.detect_singing(
                self.results["y"],
                self.results["sr"],
                self.results["feature_df"],
                singing_ref,
                non_singing_ref,
    ...
    def _process_segments(self) -> bool:
        ...
        try:
            ...
            filtered_segments = segment_filter.process_segments(
                ...
                merge_threshold=self.config.get(
                    "merge_threshold", 0.5
                ),  # Example param, adjust if needed
                verbose=False,
            )

            # Convert final list of tuples to list of Segment dataclasses
            final_segment_objects = [
                Segment(start=s[0], end=s[1]) for s in filtered_segments
            ]
            self.results["final_segments"] = final_segment_objects

            self._update_status(
                f"Processed segments: {len(filtered_segments)} final segments remaining."
            )
            return True
        except Exception as e:
    ...
    def _identify_songs(self) -> bool:
        ...
        if not final_segments:
            self._update_status(
                "Skipping song identification: No final segments to analyze."
            )
        ...
        if not api_key:
            self._update_status(
                "Warning: Gemini API key not found. Skipping song identification."
            )
            self.results["identification_results"] = []  # Ensure key exists, empty list
        ...
        try:
            ...
            self._update_progress(
                0.8, f"Identifying songs in {len(final_segments)} segments..."
            )
            # The identify_songs method now returns List[SegmentIdentification]
            identification_results: List[SegmentIdentification] = (
                self.identifier.identify_songs(
            ...
            num_identified = sum(
                ...
                and not res.identification.error
                and res.identification.title
            )
            self._update_status(
                f"Song identification attempted. Found potential titles for {num_identified} segments."
            )
            return True
        except Exception as e:
            ...
            ] = []  # Ensure key exists, empty list on error
            return False
    ...
    def run(self) -> Optional[AnalysisResults]:
        ...
        if not self._generate_base_filename():
            return None
        if self._stop_requested or not self._load_audio():
            return None
        if self._stop_requested or not self._extract_features():
            return None
        if self._stop_requested or not self._detect_segments():
            return None
        if self._stop_requested or not self._process_segments():
            return None
        if self._stop_requested or not self._identify_songs():
            # If song ID fails, maybe still return detection results?
            self._update_status(
                "Warning: Song identification failed, but returning detection results."
            )
    ...
    def visualize(self):
        ...
        feature_df = self.results.get(
            "frame_df_with_states"
        )  # Use the df with state info
        y = self.results["y"]
        sr = self.results["sr"]
        ...
        try:
            ...
            self._update_status(
                "Visualization skipped: matplotlib library not found or could not be imported."
            )
        except Exception as e:
            self._update_status(
                f"An unexpected error occurred during visualization: {e}"
            )
            import traceback
    ...
    def save_results(self):
        ...
        final_analysis_results: Optional[AnalysisResults] = self.results.get(
        ...
        if self.config.get("save_results_dataframe", False):
            try:
                ...
                elif "final_segments" in self.results and isinstance(
                    self.results["final_segments"], list
                ):
                    # Fallback to intermediate results (should be List[Segment] now)
                    final_segments_data = self.results["final_segments"]
                    if final_segments_data and not isinstance(
                        final_segments_data[0], Segment
                    ):
                        # If intermediate is somehow still tuples, handle it (though shouldn't happen)
                        self._update_status(
                            "Warning: Intermediate segments were not Segment objects, converting for CSV."
                        )
                        final_segments_data = [
                            Segment(start=s[0], end=s[1])
                            for s in final_segments_data
                ...
                if final_segments_data:
                    ...
                    csv_path = os.path.join(
                        self.run_output_dir, f"{base_filename}_segments.csv"
                    )
                    segments_df.to_csv(
                        csv_path, index_label="segment_index", float_format="%.3f"
                    )
        ...
        if self.config.get("save_results_json", False):
            try:
                ...
                if final_analysis_results:
                    ...
                    self._update_status(
                        "Skipping JSON save: Final results object not available."
                    )
                ...
                if serializable_data:
                    json_path = os.path.join(
                        self.run_output_dir, f"{base_filename}_results.json"
                    )
                    with open(json_path, "w", encoding="utf-8") as f:  # Add encoding
                        json.dump(
                            serializable_data, f, indent=4, default=str
                        )  # Use default=str for any tricky types
    ...
    def request_stop(self):
        ...
        self._update_status(
            "Stop requested. Pipeline will halt after the current step."
        )
