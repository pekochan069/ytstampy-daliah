...
class HMMDetectionEngine(DetectionEngine):
    ...
    def detect_singing(
        ...
        **params,
        ...
        non_sing_indices = np.where(
            (times >= non_sing_start) & (times <= non_sing_end)
        )[0]
        ...
        if sing_votes[0] / (non_sing_votes[0] + 0.001) > sing_votes[1] / (
            non_sing_votes[1] + 0.001
        ):
    ...
    def _find_continuous_segments(
        self, mask: np.ndarray, times: np.ndarray, min_duration: float
...
class ClusterEnhancedHMMDetectionEngine(HMMDetectionEngine):
    ...
    def detect_singing(
        ...
        **params,
        ...
        verbose = params.get("verbose", True)
        focus_on_interludes = params.get(
            "focus_on_interludes", True
        )  # Focus on interludes rather than preludes
        interlude_threshold = params.get(
            "interlude_threshold", 0.3
        )  # Threshold for interlude detection
        max_interlude_duration = params.get(
            "max_interlude_duration", 20.0
        )  # Maximum interlude duration in seconds
        ...
        if dim_reduction == "pca":
            ...
            if verbose:
                print(
                    f"Reduced features from {features_scaled.shape[1]} to {features_reduced.shape[1]} dimensions using PCA"
                )
            ...
            if verbose:
                print(
                    f"Reduced features from {features_scaled.shape[1]} to {features_reduced.shape[1]} dimensions using UMAP"
                )
        else:
            ...
            if verbose:
                print(
                    f"Using {features_scaled.shape[1]} original features without reduction"
                )
        ...
        singing_centroid = np.mean(
            features_reduced[singing_ref_indices], axis=0
        ).reshape(1, -1)
        non_singing_centroid = np.mean(
            features_reduced[non_singing_ref_indices], axis=0
        ).reshape(1, -1)
        ...
        singing_cluster_votes = np.bincount(
            cluster_labels[singing_ref_mask], minlength=n_clusters
        )
        non_singing_cluster_votes = np.bincount(
            cluster_labels[non_singing_ref_mask], minlength=n_clusters
        )

        # Calculate overlap ratios
        singing_ratio = singing_cluster_votes / np.sum(singing_cluster_votes)
        non_singing_ratio = non_singing_cluster_votes / np.sum(
            non_singing_cluster_votes
        )
        ...
        features_for_hmm = np.column_stack(
            [
                features_reduced,
                np.array(
                    [1.0 if c == singing_cluster else 0.0 for c in cluster_labels]
                ).reshape(-1, 1),
            ]
        )
            ...
            if len(segment_indices) > 0:
                ...
                try:
                    ...
                    dist_to_non_singing = self._mahalanobis_distance(
                        segment_centroid,
                        non_singing_centroid.flatten(),
                        non_singing_cov_inv,
                    )
                except:
                    # Fall back to Euclidean distance if Mahalanobis fails
                    dist_to_singing = np.linalg.norm(
                        segment_centroid - singing_centroid.flatten()
                    )
                    dist_to_non_singing = np.linalg.norm(
                        segment_centroid - non_singing_centroid.flatten()
                    )
                ...
                if total_dist > 0:
                    similarity_score = (
                        dist_to_non_singing / total_dist
                    )  # Higher when closer to singing
                else:
                    similarity_score = 0.5  # Ambiguous case

                # Also check cluster composition in this segment
                cluster_counts = np.bincount(
                    cluster_labels[segment_indices], minlength=n_clusters
                )
        ...
        singing_segments = [
            (start, end) for start, end, prob in evaluated_segments if prob >= threshold
        ]
        ...
        if len(singing_segments) > 1:
            ...
            if verbose:
                print(
                    f"After merging consecutive segments: {len(singing_segments)} segments"
                )
    ...
    def _identify_song_structure_and_interludes(
        self,
        singing_segments,
        times,
        features_reduced,
        ...
        state0_in_singing = np.mean(
            [1 if s == 0 else 0 for s in states[cluster_labels == singing_cluster]]
        )
        state1_in_singing = np.mean(
            [1 if s == 1 else 0 for s in states[cluster_labels == singing_cluster]]
        )
        ...
        for start, end in singing_segments:
            ...
            if len(segment_indices) > 0:
                ...
                segment_feats = {
                    ...
                    "cluster_pattern": self._extract_pattern(
                        cluster_labels[segment_indices]
                    ),
                }
        ...
        for i in range(len(singing_segments) - 1):
            ...
            if (
                ...
                and (next_start - current_end) <= max_interlude_duration
            ):
                ...
                if len(interlude_indices) > 0:
                    ...
                    transition_activity = np.sum(state_transitions != 0) / max(
                        1, len(interlude_indices) - 1
                    )

                    # 2. Cluster behavior in the interlude
                    singing_cluster_ratio = np.mean(
                        cluster_labels[interlude_indices] == singing_cluster
                    )
                    ...
                    interlude_score = (
                        0.4
                        * segment_similarity  # Higher when similar segments detected
                        + 0.3
                        * (
                            1 - singing_cluster_ratio
                        )  # Higher when less singing in the interlude
                        + 0.3
                        * transition_activity  # Higher with more state transitions (musical activity)
                    )
        ...
        for interlude in potential_interludes:
            if interlude["score"] >= interlude_threshold:
                ...
                if verbose:
                    print(
                        f"Identified interlude: {interlude['start']:.2f}s - {interlude['end']:.2f}s "
    ...
    def _merge_intervals_sweepline(
        self, segments: List[Tuple[float, float]], verbose=True
    ) -> List[Tuple[float, float]]:
        """
    ...
    def _compare_segment_patterns(self, segment1, segment2):
        ...
        try:
            ...
            state_sim = 1.0 / (
                1.0
                + state_distance
                / max(len(segment1["state_pattern"]), len(segment2["state_pattern"]))
            )
            ...
            cluster_sim = 1.0 / (
                ...
                + cluster_distance
                / max(
                    len(segment1["cluster_pattern"]), len(segment2["cluster_pattern"])
                )
            )

            # Duration similarity (verses/choruses often have similar duration)
            duration_ratio = min(segment1["duration"], segment2["duration"]) / max(
                segment1["duration"], segment2["duration"]
            )
            ...
            state1_counts = np.bincount(segment1["states"], minlength=2) / len(
                segment1["states"]
            )
            state2_counts = np.bincount(segment2["states"], minlength=2) / len(
                segment2["states"]
            )
            state_sim = 1 - np.mean(np.abs(state1_counts - state2_counts))

            # Compare cluster distributions
            cluster1_counts = np.bincount(segment1["clusters"], minlength=2) / len(
                segment1["clusters"]
            )
            cluster2_counts = np.bincount(segment2["clusters"], minlength=2) / len(
                segment2["clusters"]
            )
    ...
    def _visualize_cluster_enhanced_hmm(
        self,
        times: np.ndarray,
        ...
        evaluated_segments: List[Tuple[float, float, float]] = None,
    ) -> None:
        """
        ...
        num_subplots = 3
        if (
            "singing_probability" not in features_df.columns
            and evaluated_segments is None
        ):
            num_subplots -= 1
        ...
        # Plot Cluster Labels
        axes[0].scatter(
            times, cluster_labels, c=cluster_labels, cmap="viridis", alpha=0.6, s=5
        )
        axes[0].set_title("Cluster Labels")
        axes[0].set_ylabel("Cluster ID")
        axes[0].set_ylim(
            min(np.unique(cluster_labels)) - 0.5, max(np.unique(cluster_labels)) + 0.5
        )
        ...
        if "singing_probability" in features_df.columns:
            axes[2].plot(
                times, features_df["singing_probability"], alpha=0.6, color="gray"
            )
