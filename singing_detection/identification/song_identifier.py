...
try:
    ...
    try:
        ...
        print(
            f"Error: Could not import data models: {e}. Ensure 'model' is accessible."
        )
...
class SongIdentifier:
    ...
    def identify_songs(
        ...
        for i, segment in enumerate(segments):
            ...
            if segment_duration < min_segment_duration:
                if verbose:
                    print(
                        f"Skipping segment {i + 1} (too short: {segment_duration:.1f}s)"
                    )
                continue

            if verbose:
                print(
                    f"Processing segment {i + 1}: {start_time:.1f}s - {end_time:.1f}s (duration: {segment_duration:.1f}s)"
                )
            ...
            if segment_duration > max_segment_duration:
                ...
                if verbose:
                    print(
                        f"  Trimming segment for analysis to {extract_start:.1f}s - {extract_end:.1f}s ({max_segment_duration:.1f}s)"
                    )
            ...
            if not segment_path:
                ...
                identification_result = SongIdentificationResult(
            ...
            if verbose:
                print(
                    f"  Transcribing with Whisper (model: {self.whisper_model_name})..."
                )
            ...
            if not transcript:
                ...
                identification_result = SongIdentificationResult(
                    error="Transcription failed or yielded no text"
                )
                results.append(
                    SegmentIdentification(
                        segment=segment,
            ...
            if verbose:
                print("  Identifying song with Gemini...")

            identification_result = self._identify_with_gemini(
            ...
            if verbose:
                if identification_result and identification_result.title:
                    print(
                        f"  Identified as: {identification_result.title} by {identification_result.artist} (Confidence: {identification_result.confidence})"
                    )
    ...
    def _identify_with_gemini(
        self, transcript: str, start: float, end: float
    ) -> SongIdentificationResult:
        """
        ...
        try:
            ...
            try:
                ...
                return SongIdentificationResult(
                    ...
                    ),  # Use full text if no specific explanation
                )

            except (
                Exception
            ) as parse_err:  # Catch broader errors during parsing/object creation
                print(
                    f"Error parsing Gemini response or creating result object: {parse_err}"
                )
    ...
    def _call_gemini_api(self, prompt: str) -> str:
        ...
        response = requests.post(
            f"{api_url}?key={self.gemini_api_key}", headers=headers, json=data
        )
    ...
    def _parse_unstructured_response(self, response: str) -> SongIdentificationResult:
        ...
        for indicator in title_indicators:
            if indicator in response:
                ...
                if line:
                    ...
                    found_title = line.split(indicator, 1)[
                        1
                    ].strip()  # Use split with maxsplit=1
                    # Clean up any quotes
        ...
        for indicator in artist_indicators:
            if indicator in response:
                ...
                if line:
                    # Extract text after the indicator
                    found_artist = line.split(indicator, 1)[
                        1
                    ].strip()  # Use split with maxsplit=1
                    # Clean up any quotes
        ...
        for indicator in confidence_indicators:
            if indicator in response:
                ...
                if line:
                    # Extract text after the indicator
                    found_confidence = (
                        line.split(indicator, 1)[1].strip().lower()
    ...
    def _load_whisper_model(self):
        ...
        try:
            ...
            model_root = (
                bundled_model_dir if bundled_model_dir else None
            )  # whisper uses default if None
            print(
                f"Loading Whisper model '{model_name}' with download_root='{model_root}'"
            )
