...
def plot_waveform_with_segments(
    ...
    sr,
    segments,
    title="Detected Singing Segments",
...
def plot_segmentation_results(results, audio_data=None, sr=None, figsize=(14, 8)):
    ...
    for i, (start, end) in enumerate(segments):
        ...
        plt.gca().add_patch(
            Rectangle((start, y_pos), end - start, height, facecolor=color, alpha=0.8)
        )
...
def plot_singing_score_distribution(
    frame_df, segments=None, score_column="singing_score", figsize=(14, 6)
):
    """
    ...
    if segments:
        ...
        if np.any(segment_mask):
            plt.hist(
                frame_df.loc[segment_mask, score_column],
                bins=30,
                alpha=0.5,
...
def animate_detection_process(frame_df, threshold=0.6, figsize=(14, 7), interval=50):
    ...
    if (
        ...
        or not IPYTHON_AVAILABLE
        or animation is None
        or HTML is None
    ):
        print(
            "Matplotlib or IPython not available. Skipping animate_detection_process."
        )
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=len(times), interval=interval, blit=True
    )
