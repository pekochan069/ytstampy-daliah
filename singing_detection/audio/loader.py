...
class YouTubeAudioLoader(AudioLoader):
    ...
    def load_audio(self) -> Tuple[np.ndarray, int]:
        ...
        try:
            ...
            raise ImportError(
                "yt-dlp is required for YouTube download. Install with: pip install yt-dlp"
            )
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                self._file_path = os.path.join(
                    self.output_dir, f"{self._video_id}.{expected_extension}"
                )
                ...
                if not os.path.exists(self._file_path):
                    ...
                    possible_exts = [
                        ...
                        "webm",
                        "ogg",
                    ]  # Common audio extensions yt-dlp might output
                    found = False
                    for ext in possible_exts:
                        potential_path = os.path.join(
                            self.output_dir, f"{self._video_id}.{ext}"
                        )
                    ...
                    if not found:
                        raise FileNotFoundError(
                            f"Downloaded audio file not found for video ID {self._video_id} at expected path: {self._file_path}"
                        )
...
class AudioLoaderFactory:
    ...
    @staticmethod
    def create_loader(
        source: str, output_dir: Optional[str] = None, sr: Optional[int] = None
    ) -> AudioLoader:
        """
        ...
        """
        if (
            source.startswith(("http://", "https://", "www."))
