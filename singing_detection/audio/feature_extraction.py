...
class SpectralFeatureExtractor(FeatureExtractor):
    ...
    def extract_features(self, y: np.ndarray, sr: int) -> pd.DataFrame:
        ...
        print(
            f"Spectral feature extraction with frame_length={self.frame_length}, hop_length={self.hop_length}"
        )
...
class MFCCFeatureExtractor(FeatureExtractor):
    ...
    def extract_features(self, y: np.ndarray, sr: int) -> pd.DataFrame:
        ...
        print(
            f"MFCC feature extraction with frame_length={self.frame_length}, hop_length={self.hop_length}"
        )
...
class HarmonicFeatureExtractor(FeatureExtractor):
    ...
    def extract_features(self, y: np.ndarray, sr: int) -> pd.DataFrame:
        ...
        print(
            f"Harmonic feature extraction with frame_length={self.frame_length}, hop_length={self.hop_length}"
        )
        ...
        S_harmonic = np.abs(
            librosa.stft(
                y_harmonic, n_fft=self.frame_length, hop_length=self.hop_length
            )
        )
        S_percussive = np.abs(
            librosa.stft(
                y_percussive, n_fft=self.frame_length, hop_length=self.hop_length
            )
        )
...
class PitchFeatureExtractor(FeatureExtractor):
    ...
    def extract_features(self, y: np.ndarray, sr: int) -> pd.DataFrame:
        ...
        print(
            f"Pitch feature extraction with frame_length={self.frame_length}, hop_length={self.hop_length}"
        )
...
class FeatureExtractorFacade:
    ...
    def extract_all_features(self, y, sr):
        ...
        if self.frame_length is None:
            ...
            print(
                f"Setting frame_length to {self.frame_length} samples ({self.window_size_seconds} seconds)"
            )
        ...
        if self.include_pitch:
            self.extractors.append(
                PitchFeatureExtractor(self.frame_length, self.hop_length)
            )

        # Calculate time points for frames
        frame_time = librosa.frames_to_time(
            np.arange(1 + len(y) // self.hop_length), sr=sr, hop_length=self.hop_length
        )
        ...
        print(
            f"Feature extraction: {len(frame_time)} frames at {self.feature_rate:.1f} Hz"
        )
        ...
        for extractor in self.extractors:
            ...
            if len(feature_df) == len(df):
                ...
                print(
                    f"Warning: {extractor.__class__.__name__} produced {len(feature_df)} frames, expected {len(df)}"
                )
    ...
    def get_feature_rate(self):
        ...
        if self.feature_rate is None:
            raise ValueError(
                "Feature rate not available. Run extract_all_features first."
            )
