...
try:
    ...
    warnings.warn(
        "UMAP not installed. Only PCA will be available for dimensionality reduction."
    )
...
class SegmentProcessor(ABC):
    ...
    def process_segments(
        self, segments: List[Tuple[float, float]], y: np.ndarray, sr: int, **params
    ...
    def _find_energy_transition(
        ...
        if direction == "forward":
            ...
            baseline_energy = np.mean(
                y[max(0, sample_position - window_samples) : sample_position] ** 2
            )
            ...
            baseline_energy = np.mean(
                y[sample_position : min(len(y), sample_position + window_samples)] ** 2
            )
...
class SegmentFilter(SegmentProcessor):
    ...
    def process_segments(
        ...
        filtered_segments = (
            list(map(tuple, segments_array[long_mask].tolist()))
            if np.any(long_mask)
            else []
        )
        ...
        if np.any(short_mask):
            ...
            if len(filtered_segments) > 1 and not np.all(
                np.diff(segments_array[:, 0]) >= 0
            ):
        ...
        if verbose:
            print(
                f"After filtering: {len(filtered_segments)}/{len(segments)} segments kept"
            )
            if len(filtered_segments) != len(segments):
                print(
                    f"Removed {len(segments) - len(filtered_segments)} segments shorter than {min_duration}s"
                )
...
class SegmentValidator(SegmentProcessor):
    ...
    def process(self, segments, y, sr, **kwargs):
            ...
            for start, end in segments:
                ...
                if np.sum(segment_mask) > 0:
                    ...
                    if confidence >= threshold:
                        ...
                        print(
                            f"Validated segment {start:.1f}-{end:.1f}s (conf: {confidence:.2f})"
                        )
                    else:
                        print(
                            f"Rejected segment {start:.1f}-{end:.1f}s (conf: {confidence:.2f})"
                        )
        ...
        print(
            "WARNING: No detection results available, performing independent validation"
        )
