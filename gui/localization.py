...
class LocalizationManager:
    ...
    def __init__(self, locales_dir="locales", default_lang="en"):
        ...
        project_root = os.path.dirname(
            script_dir
        )  # Assumes gui/ is one level down from root
        ...
        if detected_lang in self.translations:
            ...
            first_available = (
                next(iter(self.translations.keys())) if self.translations else "en"
            )
            print(
                f"Warning: Detected ('{detected_lang}') and default ('{self.default_lang}') languages not available. Falling back to '{first_available}'."
            )
    ...
    def set_language(self, lang_code: str):
        ...
        if lang_code in self.translations:
            ...
            print(
                f"Warning: Language '{lang_code}' not found or not loaded. Keeping '{self.current_lang}'."
            )
    ...
    def tr(self, key: str, **kwargs) -> str:
        ...
        try:
            ...
            print(
                f"Error formatting translation for key '{key}' with args {kwargs}: {e}. Returning raw: {translation}"
            )
